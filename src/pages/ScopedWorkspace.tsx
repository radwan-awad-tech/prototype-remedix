...
export const ScopedWorkspace: React.FC<{ path: string }> = ({ path }) => {
  ...
  useEffect(() => {
    ...
    const paths = ROLE_ACCESS_POLICIES[user!.role].modules.map(m => m.path).filter(p => !!columns[p] || (p === '/health' && user!.role === 'Occupational Health Officer'));
  ...
  const canReview = (row: Record<string, any>) => ['HR Manager','Department Head'].includes(user!.role) && row.status === 'Pending' && row.employeeId !== user!.employeeId && row.stage === (user!.role === 'Department Head' ? 'Manager' : 'HR');
  ...
  return <section className="space-y-6">
    ...
    {loading ? <p role="status">{say('جارٍ تحميل السجلات المسموح بها…','Loading authorized records…')}</p> : overview ? <>
      ...
      {focusChart.length > 0 && <div className="card-base p-6"><h2 className="font-bold mb-4">{focusPath === '/payroll' ? say('صافي الرواتب حسب الفترة — أرقام تجريبية','Net payroll by period — demo amounts') : focusPath === '/health' ? say('متابعة مواعيد الفحوص حسب الحالة','Health check appointments by status') : say('الإجازات حسب الحالة ومرحلة الموافقة','Leave by status and approval stage')}</h2><div className="h-60" dir="ltr"><ResponsiveContainer width="100%" height="100%"><BarChart data={focusChart}><CartesianGrid vertical={false} strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:11}}/><YAxis/><Tooltip/><Bar dataKey="value" fill="#0E7875" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div></div>}
      {counts.length > 0 ? <div className="card-base p-6"><h2 className="font-bold mb-5">{say('حجم السجلات ضمن نطاقك — ليست مؤشرات سريرية','Record volume within your scope — not clinical metrics')}</h2><div className="h-72" dir="ltr"><ResponsiveContainer width="100%" height="100%"><BarChart data={counts}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fontSize:11}}/><YAxis allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#004D4D" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div></div> : <div className="card-base p-6"><p>{say('حساب تقني: لا يتم تحميل بيانات موظفين أو رواتب أو ملفات صحية.','Technical account: no employee, salary or health records are loaded.')}</p><button onClick={() => navigate('/admin')} className="mt-4 bg-brand-deep-teal text-white px-5 py-3 rounded-xl">{say('إدارة النظام','Administration')}</button></div>}
      ...
      {rows.length > 0 && <div className="card-base p-5"><h2 className="font-semibold mb-3">{say('توزيع حالات السجلات المتاحة','Status of accessible records')}</h2><div className="h-44" dir="ltr"><ResponsiveContainer width="100%" height="100%"><BarChart data={stateCounts}><XAxis dataKey="name"/><YAxis allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#0E7875" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></div></div>}
      {path === '/payroll' && user!.role === 'Payroll Officer' && <form className="card-base p-4 flex gap-3 flex-wrap" onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); void perform(() => payrollService.createRun(String(f.get('period')))); }}><label>{say('دورة جديدة','New run')} <input type="month" name="period" required className="border rounded-lg p-2 mx-2"/></label><button disabled={busy} className="bg-brand-deep-teal text-white rounded-lg px-5 py-2">{say('إنشاء','Create')}</button></form>}
      ...
      {path === '/payroll' && user!.role === 'Payroll Officer' && ['Draft','Calculated','Locked'].includes(row.status) && <button disabled={busy} className="border rounded-lg px-3 py-2" onClick={() => void perform(() => row.status === 'Draft' ? payrollService.calculateRun(row.id) : row.status === 'Calculated' ? payrollService.lockRun(row.id) : payrollService.unlockRun(row.id))}>{row.status === 'Draft' ? say('حساب','Calculate') : row.status === 'Calculated' ? say('قفل للمراجعة','Lock for review') : say('إعادة للتحضير','Return to preparation')}</button>}
      {path === '/payroll' && user!.role === 'Payroll Officer' && row.status === 'Calculated' && <button disabled={busy} className="border rounded-lg px-3 py-2" onClick={() => void perform(() => payrollService.calculateRun(row.id))}>{say('إعادة الحساب','Recalculate')}</button>}
