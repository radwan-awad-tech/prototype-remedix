...
export const ScopedWorkspace: React.FC<{ path: string }> = ({ path }) => {
  ...
  return <section className="space-y-6">
    <header className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-semibold text-brand-deep-teal">{ar ? profile.ar : user!.role}{user!.department && ` / ${user!.department}`}</p><h1 className="text-3xl font-bold mt-1">{MODULE_NAMES[path]?.[ar ? 0 : 1]}</h1></div><div className="flex items-center gap-3">{getModuleAccess(user!.role, '/access') && <button onClick={() => navigate('/access')} className="text-sm text-brand-deep-teal underline">{say('الصلاحيات','Access')}</button>}<button onClick={() => setVersion(v => v+1)} disabled={loading || busy} className="p-3 border rounded-xl bg-white" aria-label={say('تحديث','Refresh')}><RefreshCw size={20}/></button></div></header>
